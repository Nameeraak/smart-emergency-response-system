...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($captcha !== $expected) {
    ...
    $st = mysqli_prepare($conn, "SELECT * FROM hospital WHERE email=? AND status='active'");
    ...
    if ($row && password_verify($pass, $row['password'])) {
      ...
      $_SESSION['hospital_hosp'] = $row['hospital_name'];
      header("Location: dashboard.php");
      exit();
    }
...
<body class="login-bg-hospital">
  ...
  <div class="form-card glass-card">
    <div class="form-logo-pro" style="text-align:center;">
      <div class="icon-wrap-pro" style="margin:0 auto 16px; background: rgba(0,0,0,0.2); color: #fff; border: 1px solid rgba(255,255,255,0.2); background: linear-gradient(135deg,var(--green),#16a34a); box-shadow: 0 4px 15px rgba(0,0,0,0.3); font-size: 1.5rem;">🏥</div>
      <h2>Hospital Portal</h2>
      <p>ER staff emergency access</p>
    </div>
    ...
    <form method="POST"><?= csrf_field() ?>
      <div class="form-group">
        ...
        <div class="input-icon-wrap">
          ...
          <input type="email" name="email" placeholder="hospital@emergency.com" required autofocus>
        </div>
      </div>
      <div class="form-group">
      ...
      <button type="submit" class="btn btn-success btn-full btn-lg">🔓 Login as Hospital</button>
    </form>
