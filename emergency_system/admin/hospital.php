...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['add_record'])) {
    ...
    if(!$n||!$e||!$ph||!$pw) { $error="All required fields must be filled."; }
        ...
        $chk=mysqli_prepare($conn,"SELECT id FROM hospital WHERE email=?");
        ...
        if(mysqli_stmt_num_rows($chk)>0) { $error="Email already exists."; }
            ...
            $ins=mysqli_prepare($conn,"INSERT INTO hospital(name,email,phone,hospital_name,hospital_address,specialization,password) VALUES(?,?,?,?,?,?,?)");
...
if(isset($_GET['toggle'])&&is_numeric($_GET['toggle'])){
    $tid=(int)$_GET['toggle'];
    $cur=mysqli_fetch_assoc(mysqli_query($conn,"SELECT status FROM hospital WHERE id=$tid"))['status'];
    mysqli_query($conn,"UPDATE hospital SET status='".($cur==='active'?'inactive':'active')."' WHERE id=$tid");
    header("Location: hospital.php"); exit();
}
if(isset($_GET['delete'])&&is_numeric($_GET['delete'])){
    $st=mysqli_prepare($conn,"DELETE FROM hospital WHERE id=?"); $d=(int)$_GET['delete'];
    mysqli_stmt_bind_param($st,'i',$d); mysqli_stmt_execute($st);
    header("Location: hospital.php"); exit();
}
$records=mysqli_query($conn,"SELECT * FROM hospital ORDER BY created_at DESC");
$labels=explode(',','Hospital Name,Address,Specialization');
$fields=explode(',','hospital_name,hospital_address,specialization');
?>
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Manage Hospitals — Admin</title><link rel="stylesheet" href="../assets/css/style.css?v=5.0"></head>
...
<div class="layout">
  <div class="sidebar">
    ...
    <div class="sidebar-nav">
      ...
      <a href="hospital.php" class="active"><span class="nav-icon">🏥</span> Hospital</a>
    </div>
    <div class="sidebar-footer"><a href="../logout.php"><span class="nav-icon">🚪</span> Logout</a></div>
  </div>
  <div class="main-content">
    <div class="page-header">
      <div class="page-header-left"><h1>🏥 Manage Hospitals</h1><p><?=mysqli_num_rows($records)?> records</p></div>
    </div>
    ...
    <div class="card" style="margin-bottom:20px;">
      ...
      <form method="POST"><?=csrf_field()?>
        ...
        <button type="submit" name="add_record" class="btn btn-success" style="margin-top:12px;">➕ Add Record</button>
      </form>
    </div>
        ...
        <table>
          ...
          <tr>
            ...
            <td style="white-space:nowrap;">
              <a href="hospital.php?toggle=<?=$r['id']?>" class="btn btn-warning btn-xs"><?=$r['status']==='active'?'🔒 Disable':'🔓 Enable'?></a>
              <a href="hospital.php?delete=<?=$r['id']?>" class="btn btn-danger btn-xs" onclick="return confirm('Delete this record?')">🗑</a>
            </td>
          </tr>
          <?php endwhile; ?>
          </tbody>
