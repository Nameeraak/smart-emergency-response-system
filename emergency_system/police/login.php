<?php
require_once '../db.php';
if (isset($_SESSION['police_id'])) { header("Location: dashboard.php"); exit(); }
$error = '';
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($captcha !== $expected) {
        ...
        $st=mysqli_prepare($conn,"SELECT * FROM police WHERE email=? AND status='active'");
        mysqli_stmt_bind_param($st,'s',$email); mysqli_stmt_execute($st);
        $row=mysqli_fetch_assoc(mysqli_stmt_get_result($st));
        if ($row && password_verify($pass,$row['password'])) {
            $_SESSION['police_id']=$row['id']; $_SESSION['police_name']=$row['name'];
            $_SESSION['police_station']=$row['station_name']; $_SESSION['police_badge']=$row['badge_number'];
            header("Location: dashboard.php"); exit();
        }
        $error="Invalid police credentials.";
    }
}
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Police Login</title><link rel="stylesheet" href="../assets/css/style.css?v=5.0"></head><body class="login-bg-police">
...
<div class="nav-links">
    ...
    </button><a href="../index.php">← Home</a></div></nav>
<div class="form-card glass-card">
  <div class="form-logo-pro" style="text-align:center;">
    <div class="icon-wrap-pro" style="margin:0 auto 16px; background: rgba(0,0,0,0.2); color: #fff; border: 1px solid rgba(255,255,255,0.2); background: linear-gradient(135deg,var(--blue),#2563eb); box-shadow: 0 4px 15px rgba(0,0,0,0.3); font-size: 1.5rem;">🚔</div>
    <h2>Police Portal</h2>
    <p>Law enforcement dispatch</p>
  </div>
  ...
  <form method="POST"><?=csrf_field()?>
    <div class="form-group">
      ...
      <div class="input-icon-wrap">
        ...
        <input type="email" name="email" placeholder="police@emergency.com" required autofocus>
      </div>
    </div>
    <div class="form-group">
    ...
    <button type="submit" class="btn btn-info btn-full btn-lg">🔓 Login as Officer</button>
  </form>
  <div class="form-link"><a href="../login.php">← Back to User Login</a></div>
  </div>
<script src="../assets/js/app.js?v=2.1"></script></body></html>
