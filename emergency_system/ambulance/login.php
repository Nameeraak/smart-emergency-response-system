<?php
require_once '../db.php';
if (isset($_SESSION['ambulance_id'])) { header("Location: dashboard.php"); exit(); }
$error = '';
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($captcha !== $expected) {
        ...
        $st=mysqli_prepare($conn,"SELECT * FROM ambulance WHERE email=? AND status='active'");
        mysqli_stmt_bind_param($st,'s',$email); mysqli_stmt_execute($st);
        $row=mysqli_fetch_assoc(mysqli_stmt_get_result($st));
        if ($row && password_verify($pass,$row['password'])) {
            $_SESSION['ambulance_id']=$row['id']; $_SESSION['ambulance_name']=$row['name'];
            $_SESSION['ambulance_vehicle']=$row['vehicle_number'];
            header("Location: dashboard.php"); exit();
        }
        $error="Invalid ambulance credentials.";
    }
}
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Ambulance Login</title><link rel="stylesheet" href="../assets/css/style.css?v=5.0"></head><body class="login-bg-ambulance">
  ...
  <form method="POST"><?=csrf_field()?>
    <div class="form-group">
      ...
      <div class="input-icon-wrap">
        ...
        <input type="email" name="email" placeholder="ambulance@emergency.com" required autofocus>
      </div>
    </div>
    <div class="form-group">
    ...
    <button type="submit" class="btn btn-primary btn-full btn-lg">🔓 Login as Driver</button>
  </form>
